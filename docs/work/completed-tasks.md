...
## T-126 (sprint 10)
- **Description:** Rebuilt reliable packet delivery as a source-pinned, one-slot stop-and-wait state machine with prepare/commit/abort initial sends, transactional retries, exact duplicate handling, and wrap-safe reorder refusal. `RadioLink` now commits initial, ACK, and retry state only after exact driver writes, rejects reliable broadcast peers, and refuses reliability-mode changes while ARQ work is pending.
...
## T-128 (sprint 10)
- **Description:** Replaced positional compliance checks with a typed `TransmissionPlan` and strict jurisdiction parsing; enforced finite/ranged frequency, occupied bandwidth, EIRP, duty, band-span, catalog bandwidth, power, and encryption rules. Mesh and tunnel sends now fail closed before driver construction and through an immutable per-datagram policy wrapper. Tunnel policy derives Carson bandwidth and 100% duty from the same normalized modem/driver sample rate.
